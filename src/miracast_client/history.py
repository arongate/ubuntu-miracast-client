...
class SessionRecord(GObject.Object):
    ...
    def from_dict(cls, data):
        ...
        stats = CastingStats(
            ...
            end_time=(
                datetime.fromisoformat(data["stats"]["end_time"])
                if data["stats"]["end_time"]
                else None
            ),
...
class SessionHistory:
    ...
    def __init__(self, history_path=None):
        ...
        if history_path:
            ...
            self.history_path = (
                Path.home() / ".local" / "share" / "ubuntu-miracast-client" / "history.json"
            )
    ...
    def add_session(self, source, device, stats):
        ...
        """
        session = SessionRecord(source=source, device=device, stats=stats, timestamp=datetime.now())
